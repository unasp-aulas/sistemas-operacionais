...
- A parte do [gerenciamento de arquivos]{.blue} fica [transparente para os usuários]{.marked} e é feita automaticamente.
:::
::: {.fragment fragment-index=2}
...
```{.cpp filename="fork_exemplo.cpp" code-line-numbers="1-16|1-3|9|14"}
#include <stdio.h>
#include <unistd.h>
...
| `seconds = time(&seconds)` | Obtém o tempo decorrido desde 01/01/1970 |
...
1. [Em execução]{.blue} (realmente usando a CPU naquele instante).
2. [Pronto]{.blue} (executável, temporariamente parado  para deixar outro processo ser executado).
3. [Bloqueado]{.blue} (incapaz de ser executado até que algum evento externo aconteça).

<center>![](images/image02-02.png){height="120"}</center>
:::
::::
