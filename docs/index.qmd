...
::: {.columns}
::: {.column width=60%}
![Baseado em <https://materialpublic.imd.ufrn.br/curso/disciplina>](images/ciclo-da-cpu-01.svg){width="120%"}
:::
::: {.column width=40%}
[Endereçamento da próxima instrução a ser executada:]{.blue} A instrução é endereçada através do envio do conteúdo do Contador de Programa (PC), que está localizado na Unidade de Controle (UC), para a memória de instruções. Esse conteúdo representa o endereço da próxima instrução a ser executada. 
:::
:::
...
::: {.columns}
::: {.column width=60%}
![Baseado em <https://materialpublic.imd.ufrn.br/curso/disciplina>](images/ciclo-da-cpu-02.svg){width="120%"}
:::
::: {.column width=40%}
[Leitura da instrução da memória de instruções:]{.blue} a memória faz a leitura de uma palavra de instrução.
:::
:::
...
::: {.columns}
::: {.column width=60%}
![Baseado em <https://materialpublic.imd.ufrn.br/curso/disciplina>](images/ciclo-da-cpu-03.svg){width="120%"}
:::
::: {.column width=40%}
[Escrita da instrução no registrador de instruções:]{.blue} a instrução lida é escrita dentro do processador no registrador de instruções. No nosso exemplo, o valor da instrução lida é 1B
:::
:::
...
::: {.columns}
::: {.column width=60%}
![Baseado em <https://materialpublic.imd.ufrn.br/curso/disciplina>](images/ciclo-da-cpu-04.svg){width="110%"}
:::
::: {.column width=40%}
[Decodificação dos operandos:]{.blue} a instrução é decodificada, ou seja, o processador passa a conhecer qual é a instrução (que operação ela executa) e quais são os operandos.

\

O decodificador é um componente de hardware fundamental sendo [responsável por traduzir as instruções em linguagem de máquina (binário) em sinais de controle que o processador entende e executa.]{.marked}
:::
:::
...
::: {.columns}
::: {.column width=60%}
![Baseado em <https://materialpublic.imd.ufrn.br/curso/disciplina>](images/ciclo-da-cpu-05.svg){width="120%"}
:::
::: {.column width=40%}
[Leitura dos operandos da memória ou dos registradores:]{.blue} os operandos fonte (dados que serão utilizados para efetuar a operação na ULA) são lidos da memória ou dos registradores e colocados nas entradas da ULA.
:::
:::
...
::: {.columns}
::: {.column width=60%}
![Baseado em <https://materialpublic.imd.ufrn.br/curso/disciplina>](images/ciclo-da-cpu-06.svg){width="120%"}
:::
::: {.column width=40%}
[Execução da instrução:]{.blue} quando a ULA recebe os operandos fonte, então o próximo passo será a execução da operação através da ULA.
:::
:::
...
::: {.columns}
::: {.column width=60%}
![Baseado em <https://materialpublic.imd.ufrn.br/curso/disciplina>](images/ciclo-da-cpu-07.svg){width="120%"}
:::
::: {.column width=40%}
[Escrita do resultado:]{.blue} a saída da ULA, que contém o resultado da operação que acabou de ser executada, é escrita no operando destino, que pode estar em algum registrador ou na memória.
:::
:::
...
[Cada CPU tem um conjunto específico de instruções]{.marked} que ela consegue executar. *Desse modo, um processador x86 não pode executar programas ARM.* Exemplo: [Instruções lógicas x86 (AND, OR, XOR, NOT) podem operar diretamente em endereços de memória, enquanto o ARM é load/store, exigindo mover dados para registradores antes da operação.]{.marked}
...
- Sistemas operacionais de computadores de grande porte, [de alto desempenho e alta disponibilidade]{.marked} [(*mainframes*)](https://www.youtube.com/watch?v=HiQOBAEfZmQ&t=119s)
- [Sistemas operacionais de servidores](https://top500.org/lists/top500/2025/11/)
- Sistemas operacionais de computadores pessoais
- Sistemas operacionais de computadores portáteis
- Sistemas operacionais embarcados
- [Sistemas operacionais de tempo real](https://developer.sony.com/posts/apache-nuttx-powers-worlds-smallest-lunar-robot-in-japans-historic-autonomous-lunar-exploration-mission)
...
> *A CPU recebe, executa, processa e envia dados binários. A base da computação é poder escrever tudo em 0 ou 1.* -- [Blau Araújo](https://www.youtube.com/live/Ej6U-qk0bdE?si=d4f4jKpQ4CAM9Yv-&t=3056)

\

Contudo o byte é uma das menores unidades de memória usadas nos computadores e celulares modernos ([1 byte, que consiste em 8 bits, é representado em hexadecimal por dois dígitos, variando de 00 a FF]{.blue})
...
::: {.columns}
::: {.column width="35%"}
Um programa são apenas números que podem estar salvos em um arquivo .txt

\

```{filename="ok.txt"}
7F454C460101010000000000
000000000200030001000000 
548004083400000000000000 
000000003400200001002800
000000000100000054000000
54800408000000000C000000
0C0000000500000000100000
B801000000BB00000000CD80
```
...
:::
::: {.column width="65%"}
Trasformando em binário executável
...
## Conceitos de sistemas operacionais {.smaller}
...
É possível editar o arquivo utilizando o programa `hexedit`
...
0000003C   54 80 04 08  00 00 00 00  0C 00 00 00  T...........
00000048   0C 00 00 00  05 00 00 00  00 10 00 00  ............
00000054   B8 01 00 00  00 BB 01 00  00 00 CD 80  ............
```
:::
::: {.column}

[O cabeçalho ELF (Executable and Linkable Format) é a estrutura inicial de arquivos binários no Linux]{.blue}, funcionando como um "cartão de visitas" que informa ao sistema operacional como carregar e executar o programa. [Ele contém metadados essenciais, incluindo o tipo de arquivo, a arquitetura da máquina (32/64 bits) e o ponto de entrada de execução.]{.marked}

\

A ultima linha `B8 01 00 00  00 BB 01 00  00 00 CD 80` contém as [instruções do programa.]{.blue} 

- B8: MOV EAX
- 01 00 00 00: 1 (chamada de sistema de número 1 sys_exit)
- BB: MOV EBX
- 00 00 00 00: 0 (código de status de saída)
- CD: INT
- 80: 0x80 (instrução de invocação da chamada de sistema)
:::
:::
...
![](images/chamada-de-sistema.png){width=50%}
...
```
- A lista de chamadas existentes [aqui](https://www.chromium.org/chromium-os/developer-library/reference/linux-constants/syscalls/)
...
```{.c filename="write.c" code-line-numbers="1|3-6"}
#include<unistd.h>
...
O comando `grep` no Linux é uma ferramenta poderosa para buscar padrões de texto dentro de arquivos ou na saída de outros comandos. Exemplos essenciais incluem pesquisar uma palavra (`grep "termo" arquivo`)

- Ignorar maiúsculas/minúsculas (case insensitive): `grep -i "erro" arquivo.log`
- Buscar recursivamente em diretórios: `grep -r "config" /etc/`
- Mostrar número da linha: `grep -n "padrão" arquivo.txt`
- Contar ocorrências: `grep -c "erro" arquivo.log`
:::
...
## Chamada de sistema {.smaller}

\

Aqui um exemplo de como o `strace` pode detectar um erro em uma chamada de sistema. No primeiro caso a `pasta` não estava vazia, então a função `rmdir pasta` para remover a pasta produz um erro. No segundo caso a pasta estava vazia então o comando executou sem erros. Comando `strace -c rmdir pasta`.

::: {.columns}
::: {.column}
```
rmdir: failed to remove 'pasta': Directory not empty
% time     seconds  usecs/call     calls    errors syscall
------ ----------- ----------- --------- --------- --------------
 43.50    0.000184           6        30        12 openat
 17.49    0.000074           3        22           mmap
 14.18    0.000060           3        20           close
 10.64    0.000045           2        18           fstat
  8.98    0.000038           9         4           write
  5.20    0.000022          22         1         1 rmdir # <1>
  ...
  0.00    0.000000           0         1           rseq
------ ----------- ----------- --------- --------- --------------
100.00    0.000423           3       116        14 total
```

1. Erro na chamada de sistema `rmdir`
:::
::: {.column}
```
...
% time     seconds  usecs/call     calls    errors syscall
------ ----------- ----------- --------- --------- --------------
 35.60    0.000267           8        30        12 openat
 22.93    0.000172           7        22           mmap
 17.87    0.000134         134         1           rmdir #<2>
 14.13    0.000106           5        20           close
  9.47    0.000071           3        18           fstat
  0.00    0.000000           0         3           read
  0.00    0.000000           0         3           mprotect
  0.00    0.000000           0         1           munmap
  0.00    0.000000           0         3           brk
  ...
  0.00    0.000000           0         1           rseq
------ ----------- ----------- --------- --------- --------------
100.00    0.000750           6       112        13 total
```

2. Chamada de sistema ``rmdir`` sem erro
:::
:::

[Por conta do erro uma outra chamada de sistema foi requisita. Por que?]{.marked}
...
### C
```{.c filename="fork_exemplo.c" code-line-numbers="|2|6|11|15-16"}
#include <stdio.h>
#include <unistd.h>
...
### C
```{.c filename="waitpid_exemplo.c" code-line-numbers="1-6|9-10|13-16|18-21"}
#include <stdio.h>
#include <unistd.h>
#include <sys/wait.h>
int i = 1;
...
## Diferenças entre links e links simbólicos^[<https://askubuntu.com/questions/108771/what-is-the-difference-between-a-hard-link-and-a-symbolic-link>]

\

- Soft Links (links simbólicos): Você pode fazer links para arquivos e diretórios. Se a cópia real for excluída, o link não funcionará.
- Hard links (somente links): são apenas para arquivos; se a cópia real for excluída, o link funcionará, porque ele acessa os dados subjacentes que a cópia real estava acessando
...
```{.bash}
sudo su # acessar o usuário root
sudo adduser nome_do_usuario # criar um novo usuário
su - nome_do_usuario # logar com outro usuário
sudo groupadd nome_do_grupo # criar um novo grupo
sudo usermod -aG nome_do_grupo nome_do_usuario # adicionar um usuário ao grupo
```
...
> O [Redox OS](https://www.redox-os.org/) segue os princípios da filosofia Unix, integrando conceitos de [microkernel]{.blue}. [Isso significa que o núcleo do sistema apenas gerencia interações entre processos e recursos, enquanto funcionalidades adicionais são movidas para bibliotecas externas]{.marked}, que podem ser usadas tanto pelo kernel quanto por aplicativos de usuários. Um dos grandes diferenciais é que todos os drivers são executados no espaço de usuário, em ambientes isolados, o que aumenta a segurança e a estabilidade do sistema.^[<https://sempreupdate.com.br/linux/distribuicoes/lancamento-redox-os-0-9>]
...
2. `!gcc foo.c` e `foo.c` não existe no diretório
...
def main() -> int:
    try:
        ...
        print(f"fork: {err}", file=sys.stderr)
        return 1
    ...
    # ... and after that, parent waits child exit status and prints a message.
    child_pid, status = os.waitpid(pid, 0)
    if os.WIFEXITED(status):
        print(
            f"\n\t[{os.getpid()}]\tProcess {child_pid} exited with status {os.WEXITSTATUS(status)}.",
            file=sys.stderr,
        )
...
:::
::: {.column}
**Ao lado temos uma rotina para a criação de um zombie.**^[<https://stackoverflow.com/questions/25172425/create-zombie-process>] Para identificar o processo zombie, podemos utilizar o comando `ps <pid do filho>` ou o comando `top` ao clicar na letra `o` e filtrar com `S=Z`. No exemplo foi utilizado `ps 7246`
...
<center>
![](images/tanenbaum-02-39.png){width=60%}
</center>
...
Outro componente envolvido na função de scheduling da CPU é o [despachante]{.blue}. O despachante [é o módulo que passa o controle da CPU ao processo selecionado pelo scheduler]{.marked}. Essa função envolve o seguinte:
...
<center>
![](images/tanenbaum-03-11.png){width="90%"}
</center>
:::
...
## Incidente da Impressora Xerox {background-image="images/richard-stallman.jpg" background-position="250% 50%" background-size="80%" .smaller}


::: {.columns}

::: {.column width=47%}
- [O Cenário:]{.blue} No final da década de 70, no laboratório de IA do MIT, [Stallman]{.marked} deparou-se com atolamentos de papel constantes na nova impressora a laser doada pela Xerox
- [O Impedimento:]{.blue} Ele desejava modificar o software da impressora para enviar alertas de erro aos usuários, mas descobriu que a Xerox fornecia o software apenas em formato binário legível por máquinas, escondendo o código-fonte
- [O Conflito:]{.blue} Stallman buscou o código com um colega pesquisador, que recusou o compartilhamento alegando ter assinado um Acordo de Não Divulgação (NDA) com a Xerox
- [A Lição:]{.blue} Essa experiência ensinou a Stallman que os NDAs tinham vítimas e que o software proprietário sabotava a cooperação comunitária ao isolar os programadores
- [A Decisão:]{.blue} Esse dilema moral foi o catalisador para Stallman decidir que nunca faria outras pessoas vítimas desse sistema, motivando-o a criar o GNU para garantir a liberdade dos usuários
:::
::: {.column width=53%}
:::
:::
...
## Projeto GNU — Origem e Filosofia
- [Lançamento:]{.blue} Richard Stallman anunciou o projeto em setembro de 1983 para criar um sistema operacional compatível com Unix que fosse inteiramente software livre
- [Significado:]{.blue} O nome é um acrônimo recursivo para ["GNU's Not Unix"]{.marked}
- [O Manifesto GNU (1985):]{.blue} Estabeleceu a base ideológica do movimento, defendendo que o compartilhamento de software é um ato fundamental de amizade e solidariedade entre programadores

- [As Quatro Liberdades Essenciais:]{.blue}
...
4. Distribuir versões modificadas para beneficiar a comunidade

## Sistema GNU e a Lacuna do Kernel
- [Construção do Sistema:]{.blue} Durante a década de 80, o projeto desenvolveu componentes cruciais como o editor Emacs, o compilador GCC e o depurador GDB
- [A Estratégia:]{.blue} Para evitar o controle por terceiros, Stallman criou a GNU General Public License (GPL), usando a lei de direitos autorais para garantir a liberdade contínua do software (Copyleft)
- [O Kernel Hurd:]{.blue} O projeto iniciou seu próprio núcleo, o Hurd, em 1990, mas seu design ambicioso de microkernel resultou em um desenvolvimento muito lento
- [Estado em 1991:]{.blue} O sistema GNU estava quase completo, possuindo quase todas as ferramentas necessárias, exceto um núcleo funcional para gerenciar o hardware

## Kernel Linux {background-image="images/linus-torvalds.webp" background-position="105% 50%" background-size="40%"}
#### Um Hobby que Cresceu
::: {.columns}
::: {.column width=65%}
- [Origem:]{.blue} Anunciado por [Linus Torvalds]{.marked}, um estudante finlandês, em agosto de 1991 na Usenet como um projeto pessoal
- [Contexto:]{.blue} Foi desenvolvido inicialmente para computadores com processador Intel 386 por ser uma arquitetura que Torvalds possuía e queria explorar
- [Motivação:]{.blue} Surgiu da insatisfação com o sistema MINIX, que era limitado e cujas modificações não eram permitidas livremente pelo seu criador, Andrew Tanenbaum
- [Integração Inicial:]{.blue} Já em sua versão 0.02, o Linux era capaz de rodar ferramentas essenciais do projeto GNU, como o interpretador bash e o compilador gcc
...
Grupos de discussão: comp.os.minix
...
Estou desenvolvendo um sistema operacional (gratuito) (apenas um hobby, não será grande e profissional como o GNU) para clones de 386 (486) AT. Este projeto está em desenvolvimento desde abril e está começando a ficar pronto. Gostaria de receber feedback sobre o que as pessoas gostam/não gostam no Minix, já que meu sistema operacional se assemelha um pouco a ele (mesmo layout físico do sistema de arquivos (por razões práticas), entre outras coisas). Atualmente, portei o bash (1.08) e o gcc (1.40) e tudo parece funcionar. Isso implica que terei algo prático em alguns meses e gostaria de saber quais recursos a maioria das pessoas gostaria. Qualquer sugestão é bem-vinda, mas não prometo implementá-las :-)

Linus (torvalds@kruuna.helsinki.fi)

PS: Sim, ele é livre de qualquer código Minix e possui um sistema de arquivos multithread. Ele NÃO é portável (usa troca de tarefas do 386 etc.) e provavelmente nunca suportará nada além de discos rígidos AT, pois são os únicos que tenho :-(.
```

## União de Forças — GNU/Linux
- [O Encontro:]{.blue} O kernel Linux preencheu a última lacuna que faltava no sistema operacional GNU, unindo as ferramentas de usuário de Stallman ao núcleo de Torvalds
- [Identidade do Sistema:]{.blue} O que muitos chamam apenas de "Linux" é, na verdade, uma combinação do sistema GNU com o kernel Linux
- [Controvérsia do Nome:]{.blue} A Free Software Foundation defende o termo "GNU/Linux" para dar crédito ao projeto que construiu a maior parte do sistema e promover a filosofia da liberdade
- [Legado:]{.blue} Essa cooperação resultou em um sistema operacional moderno e robusto que alimenta desde a infraestrutura da internet e supercomputadores até dispositivos móveis e serviços em nuvem

# Conceitos de Distribuições Linux: Base, Flavor e Remaster 
> O ecossistema Linux é composto por centenas de sistemas que, embora compartilhem peças semelhantes "embaixo do capô", diferem em sua construção e propósito

## Distribuição Base ^[Fonte: <https://www.youtube.com/watch?v=4hPQoswkRI4>]
> Uma distribuição base é aquela utilizada como alicerce para a construção de um novo projeto. As alterações feitas na base são herdadas pela distribuição que está sendo construída em cima dela

- [Definição Técnica:]{.blue} Ocorre quando um projeto utiliza o repositório e a estrutura de outro para criar um novo produto ou projeto independente

- [Exemplos Sugeridos:]{.blue}
    - Debian: Serve como base para o Ubuntu
    - Ubuntu: Serve como base para o Linux Mint
    - Arch Linux: Serve como base para o Manjaro

## Flavor (Sabor) ou Spin
> O termo Flavor (usado pela Canonical/Ubuntu) ou Spin (usado pelo Fedora) refere-se a variações oficiais de uma mesma distribuição.

- [Analogia do Sorvete:]{.blue} Se a distribuição principal é a “massa do sorvete”, as flavors são os diferentes sabores (chocolate, baunilha, etc.).

- [Características:]{.blue} Elas continuam sendo o mesmo produto, mantendo o aval da "distribuição mãe", mas com interfaces gráficas (DEs) ou objetivos diferentes.

- [Exemplos Sugeridos:]{.blue}
    - [Kubuntu:]{.blue} Ubuntu com interface KDE Plasma
    - [Xubuntu:]{.blue} Ubuntu com interface Xfce
    - [Ubuntu Studio:]{.blue} versão focada em criadores de conteúdo e músicos
    - [Fedora Spins:]{.blue} versões do Fedora com diferentes ambientes de desktop

## Remaster (ou "Refisefuqui")
> Uma remasterização acontece quando se pega um sistema original e, sem modificar o código de forma profunda, aplicam-se customizações e ajustes particulares

- [O termo "Refisefux":]{.blue} No Brasil, utiliza-se o acrônimo pejorativo "Refisefux" (Remasterização de Final de Semana e Fundo de Quintal) para descrever projetos menores que apenas trocam papéis de parede ou ícones

- [Propósito:]{.blue} Facilitar o pós-instalação, criando uma ISO já configurada com scripts e ferramentas preferidas pelo autor

- Exemplos Sugeridos:
    - [Artcraft:]{.blue} Uma remasterização do Arch Linux com forte customização de interface e scripts para Window Managers
    - [Nobara Linux (em seu início):]{.blue} Começou como uma série de customizações que o criador (GloriousEggroll) aplicava sobre o Fedora para facilitar seu uso pessoal
...
```{.bash filename="$"}
sudo apt install neovim gcc tmux wget
```
...
Como mencionado anteriormente, quando baixamos uma image usando *pull*, estamos obtendo esta imagem de um *registry* que no caso do docker o padrão é o  [dockerhub](https://hub.docker.com/). Para logar neste registro e poder enviar as nossas próprias imagens (`push`), devemos utilizar o comando `docker login -u <user> -p <password>` e em seguida `docker push <tag>`.
...
{{< video https://www.youtube.com/watch?v=XaFOJSh9riA width="85%" height="70%" >}}
...
{{< video https://www.youtube.com/watch?v=ANmCuVynSDw width="85%" height="70%" >}}